...
class ScanMatchingOdometry : public nodelet::Nodelet, public ParamServer {
public:
  ...
  ScanMatchingOdometry(ros::NodeHandle& nh): nh(nh), mt_nh("~"), private_nh("~") {}
  // virtual ~ScanMatchingOdometry() {}
  ...
  virtual void onInit()  {
    ...
    if(private_nh.param<bool>("enable_imu_frontend", false)) {
      msf_pose_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/msf_core/pose", 1, boost::bind(&ScanMatchingOdometry::msf_pose_callback, this, _1, false));
      msf_pose_after_update_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/msf_core/pose_after_update", 1, boost::bind(&ScanMatchingOdometry::msf_pose_callback, this, _1, true));
    }
    //******** Subscribers **********
    ...
    sync->registerCallback(boost::bind(&ScanMatchingOdometry::pointcloud_callback, this, _1, _2));
    imu_sub = nh.subscribe("/imu", 1024, &ScanMatchingOdometry::imu_callback, this);
    command_sub = nh.subscribe("/command", 10, &ScanMatchingOdometry::command_callback, this);
...
{
  ros::init(argc, argv, "scanMatchingodometry_node"); // 这里填入你的节点名称
  ros::NodeHandle nh;

  std::shared_ptr<radar_graph_slam::ScanMatchingOdometry> scanMatchingodometry_ptr_ = std::make_shared<radar_graph_slam::ScanMatchingOdometry>(nh);
  scanMatchingodometry_ptr_->onInit();
